...
class ApiCore(Api):
    ...
    def get_network_nsm_device(self, filters=None):
        ...
        return self._filter(response["devices"], filters or {})
    ...
    def get_ngfw_traffic(self, interval):
        ...
        if interval not in INTERVAL_VALUES:
            raise AttributeError(f"Interval unknown, must be one of {INTERVAL_VALUES}")

        params = {"mode": "net_l7", "interval": interval}
